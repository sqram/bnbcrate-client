...
export default {  
  ...
  async beforeMount () {
   ...
   this.$store.watch( state => state, async (newValue, oldValue) => {
     if (!newValue.cart.items.length)
    {
  ...
  methods: {
    ...
    updateshipping (obj) {
      this.$store.commit('checkout/SET_SHIPPING_ADDRESS', obj)
    },
    ...
    updateCrate (crate, e) { 
      // If user tries to set < 1, keep original quantity      
      let current = this.$store.state.cart.items.filter(o => o.name == crate.name)[0].quantity             
      if (e.target.value < 1) {
        console.log('-- LESS THAN 1. SETTING TO ', current)
    ...
    async _saveAddress ()
    {
      try
      {
        ...
        if (req.data.result)
        {
          ...
          this.$store.commit('user/SET_USER_DATA', { addresses })
        }
      }
      catch (e)
      {
